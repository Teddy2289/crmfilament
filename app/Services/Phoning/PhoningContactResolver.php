<?php

namespace App\Services\Phoning;

use App\Models\ArtisanProspection;
use App\Models\Client;
use App\Models\ContactParticulier;
use App\Models\ContactPartenaire;
use App\Models\Partenaire;
use App\Models\PipelineStatut;
use App\Models\Prospect;
use App\Services\Crm\PipelineStatutService;
use Illuminate\Database\Eloquent\Model;

/**
 * Resolves a phoning queue entry ('type' + 'id') to its Eloquent model,
 * and flattens that model into the array shape the phoning workflow view expects.
 */
class PhoningContactResolver
{
    public function resolveModel(string $type, int $id): ?Model
    {
        return match ($type) {
            'prospect' => Prospect::find($id),
            'artisan' => ArtisanProspection::find($id),
            'partenaire' => ContactPartenaire::find($id) ?? Partenaire::find($id),
            'particulier' => ContactParticulier::find($id),
            'client' => Client::find($id),
            default => null,
        };
    }

    public function buildContactData(Model $model, string $type): array
    {
        return match ($type) {
            'prospect' => [
                'nom' => $model->nom,
                'prenom' => null,
                'siret' => $model->siret,
                'type_pressenti' => $model->type_pressenti_label,
                'secteur_activite' => $model->secteur_activite,
                'nb_salaries' => $model->nb_salaries,
                'chiffre_affaires' => $model->chiffre_affaires
                    ? number_format($model->chiffre_affaires, 0, ',', ' ').' €'
                    : null,
                'telephone' => $model->telephone,
                'telephone_alt' => $model->telephone_alt,
                'email' => $model->email,
                'adresse' => $model->adresse,
                'ville' => $model->ville,
                'code_postal' => $model->code_postal,
                'departement' => $model->departement,
                'adresse_complete' => $model->adresse_complete,
                'interlocuteur_prenom' => $model->interlocuteur_prenom,
                'interlocuteur_nom' => $model->interlocuteur_nom,
                'interlocuteur_fonction' => $model->interlocuteur_fonction,
                'interlocuteur_telephone' => $model->interlocuteur_telephone,
                'interlocuteur_email' => $model->interlocuteur_email,
                'interlocuteur_add_nom' => $model->interlocuteur_add_nom,
                'interlocuteur_add_fonction' => $model->interlocuteur_add_fonction,
                'interlocuteur_add_telephone' => $model->interlocuteur_add_telephone,
                'interlocuteur_add_email' => $model->interlocuteur_add_email,
                'interlocuteur' => $model->interlocuteur_complet,
                'nom_interlocuteur_standard' => $model->nom_interlocuteur_standard,
                'creneaux_permanence_cse' => $model->creneaux_permanence_cse,
                'email_general_standard' => $model->email_general_standard,
                'statut_code' => $model->statut?->value,
                'statut' => strtolower($model->statut?->value ?? 'ac'),
                'statut_label' => $this->pipelineLabel('prospect', $model->statut?->value)
                    ?: $model->statut_label,
                'statut_color' => $model->statut_color,
                'statut_description' => $this->pipelineDescription('prospect', $model->statut?->value)
                    ?: $model->statut_description,
                'statut_badge_style' => $this->pipelineBadgeStyle('prospect', $model->statut?->value),
                'taux_engagement' => $model->taux_engagement,
                'priorite' => $model->type_pressenti
                    ? ucfirst(str_replace('_', ' ', $model->type_pressenti))
                    : 'Standard',
                'teleprospecteur' => $model->teleprospecteur
                    ? trim("{$model->teleprospecteur->prenom} {$model->teleprospecteur->nom}")
                    : null,
                'commercial' => $model->commercial
                    ? trim("{$model->commercial->prenom} {$model->commercial->nom}")
                    : null,
                'date_premier_contact' => $model->date_premier_contact?->format('d/m/Y'),
                'rappel_planifie_at' => $model->rappel_planifie_at?->format('d/m/Y à H:i'),
                'rappel_en_retard' => $model->rappel_est_en_retard,
                'difficile' => $model->difficile,
                'jours_depuis_contact' => $model->jours_depuis_premier_contact,
                'notes' => $model->description,
                'motif_ko' => $model->motif_ko,
                'qf_valide' => $model->qf_valide,
                'dernier_appel_ringover' => $model->getDernierAppelRingover(),
                'tous_appels_ringover' => $model->getTousAppelsRingover(),
                'id' => $model->id,
                'type' => 'prospect',
            ],
            'artisan' => [
                'nom' => $model->nom,
                'prenom' => null,
                'telephone' => $model->telephone,
                'telephone_alt' => null,
                'email' => null,
                'statut' => $model->statut_campagne->label(),
                'statut_color' => 'info',
                'priorite' => $model->priorite_segment->label(),
                'metier' => $model->corps_de_metier?->label(),
                'notes' => $model->notes,
                'id' => $model->id,
                'type' => 'artisan',
                'adresse_complete' => null,
                'interlocuteur' => null,
                'nb_salaries'     => null,
                'chiffre_affaires' => null,
                'siret'           => null,
            ],
            'partenaire' => [
                'nom' => $model->nom_retenu ?: ($model->entreprise ?: $model->nom),
                'prenom' => $model->prenom ?? null,
                'entreprise' => $model->entreprise,
                'nom_retenu' => $model->nom_retenu,
                'nomenclature_interne' => $model->nomenclature_interne,
                'type_partenaire' => $model->type,
                'telephone' => $model->telephone_direct ?? $model->telephone_mobile ?? $model->telephone_perso ?? $model->telephone,
                'telephone_alt' => $model->telephone_mobile ?? $model->telephone_perso,
                'email' => $model->email ?? $model->email_perso,
                'site_web' => $model->site_web,
                'statut_code' => is_object($model->statut) ? $model->statut->value : (string) ($model->getRawOriginal('statut') ?: $model->statut),
                'statut' => $this->pipelineLabel('partenaire', is_object($model->statut) ? $model->statut->value : (string) ($model->getRawOriginal('statut') ?: $model->statut)) ?: ($model->statut_label ?? 'À prospecter'),
                'statut_label' => $this->pipelineLabel('partenaire', is_object($model->statut) ? $model->statut->value : (string) ($model->getRawOriginal('statut') ?: $model->statut)) ?: ($model->statut_label ?? 'À prospecter'),
                'statut_description' => $this->pipelineDescription('partenaire', is_object($model->statut) ? $model->statut->value : (string) ($model->getRawOriginal('statut') ?: $model->statut)),
                'statut_badge_style' => $this->pipelineBadgeStyle('partenaire', is_object($model->statut) ? $model->statut->value : (string) ($model->getRawOriginal('statut') ?: $model->statut)),
                'statut_color' => $model->statut_color ?? 'success',
                'priorite' => $model->niveau_influence_label ?? 'Standard',
                'origine_contact' => $model->origine_contact,
                'adresse' => $model->adresse,
                'adresse_complete' => collect([$model->adresse, $model->code_postal, $model->ville])->filter()->implode(', '),
                'ville' => $model->ville,
                'departement' => $model->departement,
                'region' => $model->region,
                'code_postal' => $model->code_postal,
                'secteur_activite' => $model->secteur_activite,
                'nb_salaries' => $model->nb_salaries,
                'chiffre_affaires' => $model->chiffre_affaires,
                'siret' => $model->siret,
                'interlocuteur' => $model->fonction ?: $model->nom,
                'fonction' => $model->fonction,
                'commercial' => $model->commercial ? trim(($model->commercial->prenom ?? '').' '.($model->commercial->nom ?? '')) : null,
                'conseiller' => $model->conseiller ? trim(($model->conseiller->prenom ?? '').' '.($model->conseiller->nom ?? '')) : null,
                'parrain_marraine' => $model->parrain_marraine,
                'parrain_marraine_texte' => $model->parrain_marraine_texte,
                'parrainage_entreprise' => $model->parrainage_entreprise,
                'possibilite_permanence' => $model->possibilite_permanence,
                'replicable' => $model->replicable,
                'syndicat_majoritaire' => $model->syndicat_majoritaire,
                'date_modification_statut' => $model->date_modification_statut?->format('d/m/Y'),
                'date_signature' => $model->date_signature?->format('d/m/Y'),
                'annee_signature' => $model->annee_signature,
                'nombre_ventes_liees' => $model->nombre_ventes_liees,
                'notes' => collect([$model->commentaires, $model->notes, $model->commentaire_import])->filter()->implode("\n\n"),
                'id' => $model->id,
                'type' => 'partenaire',
            ],
            'particulier' => [
                'nom' => $model->nom,
                'prenom' => $model->prenom,
                'telephone' => $model->telephone,
                'telephone_alt' => null,
                'email' => $model->email,
                'statut' => $model->statut_occupant?->label() ?? 'Contact',
                'statut_color' => 'gray',
                'priorite' => $model->type_logement?->label() ?? 'Standard',
                'notes' => $model->adresse_complete,
                'id' => $model->id,
                'type' => 'particulier',
                'adresse_complete' => $model->adresse_complete ?? null,
                'interlocuteur' => null,
            ],
            'client' => [
                'nom' => $model->nom_tiers,
                'prenom' => $model->prenom,
                'nom_affiche' => $model->nom_affiche,
                'prenom_affiche' => $model->prenom_affiche,
                'nom_naissance' => $model->nom_naissance,
                'telephone' => $model->telephone,
                'telephone_alt' => null,
                'email' => $model->email,
                'statut' => \App\Models\Client::etatLabel($model->etat),
                'statut_code' => $model->etat,
                'statut_description' => \App\Models\Client::etatDescription($model->etat),
                'statut_color' => \App\Models\Client::etatColor($model->etat),
                'priorite' => $model->type_tiers ?? 'Standard',
                'notes' => collect([$model->entreprise, $model->notes_commerciales])->filter()->implode("\n\n"),
                'notes_commerciales' => $model->notes_commerciales,
                'adresse_complete' => $model->adresse_complete ?? null,
                'adresse' => $model->adresse,
                'code_postal' => $model->code_postal,
                'ville' => $model->ville,
                'region' => $model->region,
                'departement' => $model->departement,
                'source_sheet' => $model->source_sheet,
                'ref_client' => $model->ref_client,
                'ref_clients' => $model->ref_clients,
                'type_tiers' => $model->type_tiers,
                'entreprise' => $model->entreprise,
                'date_naissance' => $model->date_naissance?->format('d/m/Y'),
                'age' => $model->age,
                'montant_cpf' => $model->montant_cpf,
                'ne_plus_contacter' => (bool) $model->ne_plus_contacter,
                'interlocuteur' => data_get($model->extra_data, 'interlocuteur'),
                'suivi_client' => data_get($model->extra_data, 'suivi_client'),
                'statut_rattachement_partenaire' => $model->statut_rattachement_partenaire,
                'nomenclature_partenaire' => $model->nomenclature_partenaire_import,
                'partenaire_id' => $model->partenaire_id,
                'partenaire_nom' => $model->partenaire?->nom ?? $model->partenaire?->nom_prenom ?? null,
                'partenaire_entreprise' => $model->partenaire?->entreprise,
                'parrain_id' => $model->parrain_id,
                'parrain_nom' => $model->parrain?->nom_prenom ?? $model->parrain?->nom ?? null,
                'commercial' => $model->commercial ? trim(($model->commercial->prenom ?? '').' '.($model->commercial->nom ?? '')) : null,
                'id' => $model->id,
                'type' => 'client',
            ],
            default => [],
        };
    }

    protected function pipelineLabel(string $modelType, ?string $code): ?string
    {
        if (! $code) {
            return null;
        }

        return app(PipelineStatutService::class)->label($modelType, $code);
    }

    protected function pipelineDescription(string $modelType, ?string $code): ?string
    {
        if (! $code) {
            return null;
        }

        return PipelineStatut::findFor($modelType, $code)?->description;
    }

    protected function pipelineBadgeStyle(string $modelType, ?string $code): string
    {
        if (! $code) {
            return 'background:rgb(243 244 246); color:rgb(55 65 81); border:1px solid rgb(229 231 235);';
        }

        return PipelineStatut::findFor($modelType, $code)?->badge_style
            ?? 'background:rgb(243 244 246); color:rgb(55 65 81); border:1px solid rgb(229 231 235);';
    }
}
