...
class AopiaMailTemplateService
{
    public function mailConfirmationCse(Prospect $prospect, RendezVous $rdv, User $teleprospecteur): array
    {
        ...
        return [
            ...
            'subject' => 'Confirmation de votre rendez-vous '.$brand.' — '.$date->format('d/m/Y').' à '.$date->format('H:i'),
            'body' => "Bonjour {$prospect->interlocuteur_nom},\n\n".
                ...
                ($teleprospecteur->nom_complet ?? $teleprospecteur->name ?? $teleprospecteur->email).' — '.$brand,
        ];
    }
    ...
    public function mailInvitationResponsable(Prospect $prospect, RendezVous $rdv, User $teleprospecteur): array
    {
        ...
        return [
            ...
            'subject' => '[RDV '.$brand.'] '.($prospect->raison_sociale ?: $prospect->nom).' — '.$date->format('d/m/Y').' à '.$date->format('H:i').' — '.$prospect->interlocuteur_nom,
            'body' => "Bonjour,\n\n".
    ...
    {
        return app(CrmSettingsService::class)->get('theme.brand_name')
            ?? config('aopia.brand_name', config('aopia.mail.from_name', 'AOPIA Formation'));
    }

    private function brandNameLong(): string
    {
        return app(CrmSettingsService::class)->get('theme.brand_name_long')
            ?? config('aopia.brand_name_long', 'AOPIA LIKE Formation');
    }
}
