...
class ProspectImporter
{
    ...
    protected function buildColumnMapping(array $headerRow): void
    {
        $fieldAliases = [
            ...
            'cse_secretaire_email_perso' => ['secretaire Email personnel', 'cse secretaire Email personnel'],
            ...
            'cse_tresorier_email_perso' => ['tresorier Email personnel', 'cse tresorier Email personnel'],
            ...
            'syndicat_email_perso' => ['syndicat Email personnel'],
    ...
    protected function extractDepartement(?string $value): ?string
    {
        ...
        if (preg_match('/^\d{2,3}$/', trim($value))) {
            return trim($value);
        }
    ...
    protected function cleanBool(mixed $value): ?bool
    {
        ...
        if (in_array($lower, ['1', 'oui', 'yes', 'true', 'vrai'])) {
            return true;
        }
        if (in_array($lower, ['0', 'non', 'no', 'false', 'faux'])) {
            return false;
        }
