...
$table = DummyTable::make(new DummyLivewire(new Table(new class implements HasTable {
    public function getTable(): Table { throw new Exception(''); }
} )));

$livewire = new DummyLivewire($table);
$table = Table::make($livewire);
$filter = SelectFilter::make('commercial_id')->relationship('commercial','nom');
$filter->table($table);

var_dump('relationship name', $filter->getRelationshipName());
try {
    $relationship = $filter->getRelationship();
    var_dump($relationship);
    var_dump(get_class($relationship));
} catch (Throwable $e) {
