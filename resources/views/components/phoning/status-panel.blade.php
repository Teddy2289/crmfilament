@props([
    'statuts',           {{-- array — liste complète des statuts (getStatutsPhoning()) --}}
    'selectedStatut',    {{-- string — code du statut sélectionné (statut_resultat) --}}
    'commentaires',      {{-- string — texte du compte rendu --}}
    'rappelDate',        {{-- string — date du rappel (rappel_date) --}}
    'rappelHeure',       {{-- string — heure du rappel (rappel_heure) --}}
    'statutsGroupes' => [],       {{-- array — statuts groupés par cas (getStatutsPhoningGroupes()) --}}
    'rappelCodes'    => [],       {{-- array — codes déclenchant le bloc rappel --}}
    'pipelinePreview' => null,    {{-- array|null — aperçu de transition pipeline --}}
])

{{--
    Composant : status-panel
    Contient :
      - Onglets "Cas" (switchCaseTab JS)
      - Chips de sélection de statut (wire:click $set statut_resultat)
      - Aperçu de transition pipeline (optionnel)
      - Rappel box (date + heure, conditionnelle)
      - Fiches recap contextuelles (rdv, cse_ni, bloc/ncse, récap saisie)
      - Zone commentaires + messages de validation
      - Boutons Enregistrer & Passer

    Props :
      - statuts         : array  — getStatutsPhoning()
      - selectedStatut  : string — statut_resultat (Livewire property)
      - commentaires    : string — commentaires (Livewire property)
      - rappelDate      : string — rappel_date (Livewire property)
      - rappelHeure     : string — rappel_heure (Livewire property)
      - statutsGroupes  : array  — getStatutsPhoningGroupes()
      - rappelCodes     : array  — getRappelStatusCodes()
      - pipelinePreview : array|null — getPipelineTransitionPreview()
--}}

@php
    // Déterminer l'onglet actif : celui qui contient le statut sélectionné, sinon le premier.
...
<div class="pw-result-panel pw-card">
        ...
        <div class="pw-chip-group">
            <div class="pw-chip-row">
                ...
                <label wire:click="$set('statut_resultat', '{{ $option['value'] }}')"
                       ...
                       title="{{ $option['label'] }} — {{ $option['sub'] ?? '' }}{{ !empty($option['action']) ? ' → ' . $option['action'] : '' }}"
                       class="pw-chip"
                       style="{{ $optionStyle }}">
                    ...
                    <span class="pw-chip-main">
                        ...
                        <span class="pw-chip-sub" style="{{ $isActive ? 'color:rgba(255,255,255,.85);' : '' }}">
                            {{ $option['sub'] ?? '' }}{{ !empty($option['action']) ? ' → ' . $option['action'] : '' }}
                        </span>
                        @if (!empty($option['pipeline_label']))
                        <span class="pw-chip-pipeline"
                              style="{{ $isActive ? 'color:rgba(255,255,255,.9);' : 'color:rgb(100 116 139);' }}">
    ...
    <div class="pw-pipeline-link" wire:key="pipeline-preview-{{ $selectedStatut }}">
        ...
        <div class="pw-pipeline-link-note">
            ...
            @else
                Le statut pipeline du prospect passera automatiquement à
                «&nbsp;{{ $pipelinePreview['next']['label'] ?? '—' }}&nbsp;» lors de l'enregistrement.
            @endif
        </div>
    </div>
    @endif

    {{-- ── Rappel box (date + heure) ────────────────────────────────────── --}}
    <div id="pw-rappel-box"
         class="pw-rappel-box {{ in_array($selectedStatut, $rappelCodes) ? 'visible' : '' }}">
        ...
        <div style="font-size:0.7rem;background:#fffbe6;border:1px dashed #d4a800;border-radius:.5rem;padding:4px 8px;color:#7a5c00;margin-bottom:.5rem;display:flex;align-items:center;gap:.375rem;">
    ...
    {{-- Messages de validation commentaires --}}
    @if ($selectedStatut && in_array($selectedStatut, ['rapl_elu', 'rapl_std']) && !$commentaires)
    <div style="font-size:.75rem;color:rgb(220 38 38);margin-top:.5rem;display:flex;align-items:center;gap:.375rem;">
        <svg class="pw-icon-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
    ...
    <div style="font-size:.75rem;color:rgb(249 115 22);margin-top:.5rem;display:flex;align-items:center;gap:.375rem;">
    ...
    <div style="font-size:.875rem;color:rgb(190 35 50);margin-top:.75rem;background:rgb(254 226 226);border:1px solid rgb(248 113 113);border-radius:.75rem;padding:.75rem;">
        <div style="font-weight:700;margin-bottom:.5rem;">Erreur de validation</div>
        @error('statut_resultat')<div>Statut : {{ $message }}</div>@enderror
        @error('commentaires')<div>Commentaires : {{ $message }}</div>@enderror
    </div>
    @endif
    @error('commentaires')
    <div style="font-size:.75rem;color:rgb(190 35 50);margin-top:.5rem;">{{ $message }}</div>
    @enderror
    ...
    <div class="pw-actions">
        ...
        <button wire:click.prevent="skipCall"
    ...
    <div style="margin-top:.75rem;color:rgb(190 35 50);font-size:.875rem;">
        @if (!$selectedStatut)
