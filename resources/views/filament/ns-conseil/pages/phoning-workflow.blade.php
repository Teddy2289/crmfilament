...
<x-filament-panels::page>
    ...
    <div class="pw-wrap">
        ...
        <div style="display:flex; align-items:center; gap:1rem; padding:0.5rem 1rem; background:rgb(249 250 251); border-bottom:1px solid rgb(229 231 235); flex-wrap:wrap;">
            <div style="display:flex; align-items:center; gap:0.5rem; flex:1; min-width:200px; position:relative;">
                ...
                <input type="text"
                    ...
                    style="flex:1; padding:0.375rem 0.75rem; font-size:0.8125rem; border:1px solid rgb(209 213 219); border-radius:0.375rem; outline:none;">
                @if ($searchQuery)
        ...
        <div style="position:absolute; z-index:1000; background:white; border:1px solid rgb(229 231 235); border-radius:0.5rem; box-shadow:0 10px 25px rgba(0,0,0,0.15); max-height:400px; overflow-y:auto; width:100%; max-width:600px; margin-top:0.25rem;">
            ...
            <div wire:click="selectSearchResult({{ $result['id'] }}, '{{ $result['type'] }}')"
                ...
                <button onclick="event.stopPropagation(); appelerAvecRingover('{{ $result['telephone'] }}')" style="padding:0.25rem 0.75rem; background:rgb(34 197 94); color:white; border:none; border-radius:0.375rem; font-size:0.6875rem; font-weight:600; cursor:pointer; white-space:nowrap; flex-shrink:0;">
                    Appeler →
                </button>
            </div>
            @endforeach
        ...
        <div style="position:absolute; z-index:1000; background:white; border:1px solid rgb(229 231 235); border-radius:0.5rem; box-shadow:0 10px 25px rgba(0,0,0,0.15); padding:1rem; text-align:center; color:rgb(107 114 128); width:100%; max-width:600px; margin-top:0.25rem;">
        ...
        <div class="pw-body">
                ...
                <div class="pw-infos">
                    ...
                    <div class="pw-info-tabs">
                        ...
                        <button class="pw-info-tab" data-tab="journal" onclick="switchInfoTab('journal')">
                            ...
                            <span style="display:inline-flex;align-items:center;justify-content:center;min-width:1.25rem;height:1.25rem;padding:0 0.25rem;border-radius:9999px;background:rgb(99 102 241);color:white;font-size:0.65rem;font-weight:700;margin-left:0.25rem;">{{ count($callHistory) }}</span>
                            @endif
                        </button>
                        @if (($info['type'] ?? '') !== 'client')
                    ...
                    <div class="pw-info-panel" data-tab="journal" style="display:none;">
                        ...
                        <div style="display:flex; flex-direction:column; gap:0.625rem;">
                            ...
                            @endphp
                            <div style="border-radius:0.5rem; border:1px solid {{ $jColor['border'] }}; background:{{ $jColor['bg'] }}; padding:0.625rem 0.75rem;">
                                ...
                                @if ($appel['notes'])
                                <div style="font-size:0.75rem; color:rgb(75 85 99); background:rgba(255,255,255,0.6); border-radius:0.25rem; padding:0.25rem 0.5rem; margin-top:0.25rem; border-left:2px solid {{ $jColor['badge'] }};">
                                    {{ $appel['notes'] }}
                                </div>
                                @endif
                            </div>
            ...
            <div class="pw-right">
                ...
                <div wire:ignore id="ringover-embed-phoning"
                    style="width:100%; max-width:100%; height:560px; border-radius:0.5rem; overflow:hidden; box-sizing:border-box; border:1px solid rgb(229 231 235); margin-bottom:1rem;">
                </div>
            </div>

            <div style="font-size:0.6875rem; font-weight:700; text-transform:uppercase; letter-spacing:0.08em; color:rgb(100 116 139); margin-bottom:0.5rem; padding-bottom:0.375rem; border-bottom:1px solid rgb(241 245 249);">
                Résultat de l'appel
            </div>
            ...
            <div style="font-size:0.75rem; color:rgb(220 38 38); margin-top:0.5rem; display:flex; align-items:center; gap:0.25rem;">
