<div x-data="kanbanBoard()" x-init="initBoard()" class="kanban-board">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Pipeline Partenaires</h2>
        ...
        <div class="kanban-column flex-shrink-0 w-80 bg-gray-100 rounded-lg p-4" data-column="{{ $column['id'] }}">
            <div class="flex items-center gap-2 mb-4">
                ...
                <div>
                    ...
                    <span class="text-sm text-gray-500">{{ count($partenaires[$column['id']]['cards'] ?? []) }} partenaires</span>
                </div>
            </div>
            ...
            <div class="kanban-cards space-y-3 min-h-[200px]" 
                ...
                @if(isset($partenaires[$column['id']]['cards']))
                    @foreach($partenaires[$column['id']]['cards'] as $card)
                    <div class="kanban-card bg-white rounded-lg shadow-sm p-4 cursor-move hover:shadow-md transition"
                        ...
                        <div class="flex items-start justify-between mb-2">
                            <div>
                                <h4 class="font-medium text-gray-900">{{ $card['nom'] }}</h4>
...
function kanbanBoard() {
    return {
        ...
        initBoard() {
            this.$wire.on('partenaire-updated', () => {
                this.$wire.loadData();
            });
        },
