...
@endphp
<x-mail::message>
# Rapport quotidien CRM

Bonjour {{ $rapport['user']->prenom }},

Voici votre récapitulatif pour la journée du **{{ $debut->format('d/m/Y') }}**.
...
| RP — Rappels planifiés | {{ $rapport['rp'] ?? '—' }} |
...
| Indicateur | Valeur |
|:---|:---:|
| RDV réalisés | {{ $rapport['rdv_realises'] }} |
| RDV annulés | {{ $rapport['rdv_annules'] }} |
| RDV décalés | {{ $rapport['rdv_decales'] }} |
| No-show | {{ $rapport['taux_no_show'] ?? '—' }}% |
</x-mail::table>

@if (!empty($rapport['rdv_a_venir']) && $rapport['rdv_a_venir']->count() > 0)
## RDV à venir
...
<x-mail::table>
| Téléprospecteur | Appels | CSE joints | QF | Base AC |
|:---|:---:|:---:|:---:|:---:|
@foreach ($rapport['phoning_consolide'] as $tp)
| {{ $tp['user']->prenom }} {{ $tp['user']->nom }} | {{ $tp['appels_semaine'] }} | {{ $tp['cse_joints'] }} | {{ $tp['qf'] }} | {{ $tp['base_ac'] }} |
@endforeach
</x-mail::table>
@endif
...
<x-mail::table>
| Commercial | RDV jour | Réalisés | Annulés | No-show |
|:---|:---:|:---:|:---:|:---:|
@foreach ($rapport['commercial_consolide'] as $com)
| {{ $com['user']->prenom }} {{ $com['user']->nom }} | {{ $com['rdv_total'] }} | {{ $com['rdv_realises'] }} | {{ $com['rdv_annules'] }} | {{ $com['taux_no_show'] }}% |
@endforeach
</x-mail::table>
@endif
